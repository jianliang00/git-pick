...
pub(crate) fn read_entry_for_patch(path: &Path, filemode: u32) -> Result<Vec<u8>, SyncError> {
    let metadata = fs::symlink_metadata(path).map_err(|source_err| SyncError::Io {
        path: path.to_path_buf(),
        source: source_err,
    })?;
    if metadata.file_type().is_symlink() || filemode == 0o120000 {
        let target = fs::read_link(path).map_err(|source_err| SyncError::Io {
            path: path.to_path_buf(),
            source: source_err,
        })?;
        Ok(os_str_to_bytes(target.as_os_str()))
    } else {
        fs::read(path).map_err(|source_err| SyncError::Io {
            path: path.to_path_buf(),
            source: source_err,
        })
    }
...
pub(crate) fn create_temp_dir_for(context: &Path) -> Result<TempDir, SyncError> {
    TempDir::new().map_err(|source_err| SyncError::Io {
...
pub(crate) fn create_dir_all(path: &Path) -> Result<(), SyncError> {
    fs::create_dir_all(path).map_err(|source_err| SyncError::Io {
        path: path.to_path_buf(),
        source: source_err,
    })
}
...
pub(crate) fn remove_dir_all(path: &Path) -> Result<(), SyncError> {
    fs::remove_dir_all(path).map_err(|source_err| SyncError::Io {
        path: path.to_path_buf(),
        source: source_err,
    })
}
...
pub(crate) fn remove_file(path: &Path) -> Result<(), SyncError> {
    fs::remove_file(path).map_err(|source_err| SyncError::Io {
        path: path.to_path_buf(),
        source: source_err,
    })
}
